...
def find_next_trading_day(given_date):
    nyse = mcal.get_calendar('NYSE')
    # Ensure given_date is timezone-naive or make it timezone-naive
    given_date_tz_naive = pd.Timestamp(given_date).tz_localize(None)
    end_date = given_date_tz_naive + pd.Timedelta(days=10)
    valid_days = nyse.valid_days(start_date=given_date_tz_naive, end_date=end_date)

    # Convert valid_days to timezone-naive for comparison
    valid_days_naive = [day.tz_localize(None) for day in valid_days]

    for day in valid_days_naive:
        if day > given_date_tz_naive:
            return day
    return None
...
# Make prediction for the next valid trading day
...
print(f"Predicted price for {next_trading_day.strftime('%Y-%m-%d')}: {predicted_price[0][0]}")
